...
export function ParseMemoryToBytes(input) {
  // 去除空格并转为小写
  const str = input.trim().toLowerCase();

  // 正则匹配数字和单位（支持 b, kb, mb, gb）
  const match = str.match(/^(\d+(?:\.\d+)?)\s*(b|kb|mb|gb)?$/);
  if (!match) {
    throw new Error('Invalid memory format. Example: "50mb", "2gb", "1024kb"');
  }

  const value = parseFloat(match[1]);
  const unit = match[2] || 'b'; // 默认单位是字节（b）

  // 单位换算（使用 1024 进制，符合内存/存储惯例）
  const multipliers = {
    b: 1,
    kb: 1024,
    mb: 1024 ** 2,
