...
function ClashConnection() {
  ...
  DoClashReqeust('/proxies', 'GET', {}).then(function(r) {
...
function DoClashReqeust(path, method, data) {
  ...
  return MyPost(state.apiAddress + "/proxy_get2", {
...
function UpdateClashDelay(node, url) {
  ...
  DoClashReqeust("/proxies/" + encodeURIComponent(node['name']) + '/delay', 'GET', params).then(function(response) {
...
function UpdateClashGroupDelay(url) {
  ...
  DoClashReqeust("/group/proxy/delay", 'GET', params).then(function(response) {
      state.clashProxies.isUpdatingDelay = true
      for (var item in nodes) {
        var item = nodes[item]
...
function ApplyCoreConfig() {
  ...
  if (clash.enabled && clash.apiIP != '127.0.0.1' && clash.external_ui != '' && clash.apiPort != '') {
    inboudPorts.push(clash.apiPort)
  }
