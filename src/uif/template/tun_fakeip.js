...
var DNS_LOCAL = "dns::local"
var DNS_RESOVLER_LOCAL = "dns::resovler_local"
var DNS_RESOVLER_REMOTE = "dns::resovler_remote"
var DNS_FAKEIP = "dns::fakeip"
...
export var DEFAULT_REMOTE_IP_DNS = "udp://8.8.8.8"
...
var template = {
  ...
  "experimental": {
    ...
    "debug": {
      // "gc_percent": 80,
      // "memory_limit": ParseMemoryToBytes('100MB'),
    },
  },
  "dns": {
    "servers": [{
  ...
  "route": {
    ...
    "rules": [{
      "action": "reject",
      // "inbound": [TUN_TAG_FAKEIP, TUN_TAG_REALIP],
      "inbound": ["only DomainStrategy != prefer_ipv6"],
      "ip_cidr": ["::/0"],
    },
    {
      "action": "sniff",
...
export function AddTun(res, config) {
  ...
  var mtu = 0
  var mtuString = setting['mtu']
  if (mtuString != '') {
    try {
      mtu = parseInt(mtuString)
    } catch (_) {}
  }
...
export function SetSSM(res) {
  var t = uif.state.config.ssm
  if (!t.enabled) {
    return
  }
  res['services'].push({
...
export function AddRouteList(res, uifConfig, routeList, isShare) {
  ...
  for (var item in route) {
    item = route[item]
    var hasField = false
    var addNewDNSClient = false
    var dnsServerTag = DNS_LOCAL
    var outTag = item['_realOut']
    var outSelector = item['outbound']
    ...
    var usedDNSAddress = remote
    if (!(['proxy', 'freedom', 'block'].includes(outTag))) {
      addNewDNSClient = true
    }
    if (outTag == 'proxy') {
      dnsServerTag = DNS_PROXY
    }
    if (outTag == 'freedom') {
      usedDNSAddress  = local
    }

    var dnsServer = {
      "tag": dnsServerTag,
      "address": usedDNSAddress,
      "detour": outSelector,
    }

    if (addNewDNSClient) {
      dnsServerTag = "DNS::" + outSelector
      dnsServer['tag'] = dnsServerTag
    }

    var dnsRule = {
      'server': dnsServerTag,
    }
    var copyField = ['domain',
      'domain_suffix', 'domain_keyword', 'domain_regex',
      'auth_user', 'protocol', 'network', 'inbound', 'port', 'source_ip_cidr', 'process_name']
    for (var field in copyField) {
      field = copyField[field]
      if (field in item) {
        dnsRule[field] = item[field]
        hasField = true
      }
    }
    if (hasField) {
      ...
      if (addNewDNSClient) {
        res['dns']['servers'].push(dnsServer)
      }
    }
    delete item['_realOut']
  }
  if (dnsRules.length > 0) {
    InsertArray(res['dns']['rules'], 0, dnsRules)
  }
  ...
  InsertArray(res['route']['rules'], 7, route)
}
...
export function SetDNS(res, uifConfig, isShare) {
  ...
  if (ip_mode != '' && ip_mode == 'remote_without_ecs') {
    res['dns']['rules'][3]['client_subnet'] = '114.114.114.114';
    res['dns']['rules'][3]['server'] = DNS_PROXY;
  }
  ...
  for (var item in res['dns']['servers']) {
    ...
    var u = URLParser(item['address'])
    delete item['address']
    var scheme = u.protocol.replace(':', ''); // 去掉冒号
    item['type'] = scheme
    item['server'] = u.hostname
    var p = u.port
    if (p == '') {
  ...
  var fakeipSetting = uifConfig['fakeip']
  if (fakeipSetting['enabled'] && false) {
    var fakeip = {}
    var fakeRange = [fakeipSetting['inet4_range']]
    var ipv6Address = fakeipSetting['inet6_address'];
    var ipv6Range = fakeipSetting['inet6_range']
    if (ipv6Address != undefined && ipv6Address != '' && ipv6Range != undefined && ipv6Range != '') {
      fakeip['inet6_range'] = ipv6Range
      tun['address'].push(ipv6Address)
      fakeRange.push(ipv6Range)
    }
    fakeipSetting['range_cidr'] = fakeRange

    // fakeip['enabled'] = true
    fakeip['type'] = 'fakeip'
    fakeip['tag'] = DNS_FAKEIP
    fakeip['inet4_range'] = fakeipSetting['inet4_range']
    fakeip['inet6_range'] = 'fc00::/18'
    // fakeip['rewrite_ttl'] = 60

    res['experimental']['cache_file']['store_fakeip'] = fakeipSetting['store_fakeip']
    res['dns']['servers'].push(fakeip)

    var route = res['route']['rules'][7];
    route['ip_cidr'] = fakeipSetting['range_cidr']
    res['dns']['rules'].push({
      "query_type": [
        "A",
        "AAAA"
      ], "rewrite_ttl": fakeipSetting['rewrite_ttl'], "server": DNS_FAKEIP
    })
  }
}
...
export function BuildCoreConfig(uifConfig, boundConfig, useHttpApi, isShare) {
  ...
  coreConfig['route']['rules'][6]['strategy'] = domain_strategy
  if (domain_strategy != "prefer_ipv6") {
    coreConfig['route']['rules'][0]['inbound'] = [TUN_TAG_FAKEIP, TUN_TAG_REALIP]
  }
