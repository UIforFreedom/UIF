...
export var DEFAULT_DNS_LOCAL = "https://dns.alidns.com/dns-query"
export var DEFAULT_DNS_REMOTE = "https://dns.google/dns-query"
...
export function AddInboudList(inboundList, domain_strategy) {
  ...
  for (var item in inboundList) {
    ...
    if (item['protocol'] == 'tun') {
      ...
      console.log(uif.state.connection)
      if ('set_system_proxy' in temp && uif.state.connection.osType == 'linux') {
        temp['set_system_proxy'] = false
      }
    }
    i += 1
  }
...
export function AddRouteList(res, uifConfig, routeList, isShare) {
  ...
  InsertArray(res['route']['rules'], 5, route)
}
