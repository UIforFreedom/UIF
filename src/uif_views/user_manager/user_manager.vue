  ...
  <div class="app-container">
          ...
          <template slot-scope="scope">
            <el-dropdown trigger="click">
              ...
              <el-dropdown-menu slot="dropdown">
                ...
                <el-dropdown-item
                  icon="el-icon-link"
                  @click.native="AddOrUpdateUserDNSRecord(scope.row)"
                  >{{
                    $translator({ cn: "新增/更新解析", en: "Add Or Update DNS" })
                  }}</el-dropdown-item
                >

                <el-dropdown-item
                  icon="el-icon-close"
                  @click.native="DeleteUserDNSRecord(scope.row)"
                  >{{
                    $translator({ cn: "删除解析", en: "Delete DNS" })
                  }}</el-dropdown-item
                >

                <el-dropdown-item
                  icon="el-icon-picture-outline"
                  @click.native="ShowShareQRCode(scope.row, 'subscription')"
                  >{{
                    $translator({ cn: "订阅二维码", en: "Config" })
                  }}</el-dropdown-item
                >

                <el-dropdown-item
                  icon="el-icon-picture-outline"
                  @click.native="ShowShareQRCode(scope.row, 'vmess')"
                  >{{
                    $translator({ cn: "Vmess二维码", en: "Config" })
                  }}</el-dropdown-item
                >
    ...
    <el-card class="box-card">
      ...
      >
        ...
        <el-table-column
          prop="dispatch_domain"
          :label="$translator({ cn: '订阅分发域名', en: 'Dispatch Domain' })"
          min-width="180"
          align="center"
        >
        </el-table-column>
        ...
        <el-table-column label="操作" width="100" align="center">
          <template slot-scope="scope">
            <el-button type="primary" size="mini" @click="handleDomainDetail(scope.row)">
              配置
            </el-button>
          </template>
        </el-table-column>
    ...
    >
      <add_domain @saved="handleDomainSaved" />
    </el-dialog>
    ...
    >
      <div class="share-qr-dialog">
        <img
          v-if="qrDialog.imageDataUrl"
          :src="qrDialog.imageDataUrl"
          :alt="qrDialog.title"
          class="share-qr-image"
        />
        <div class="share-qr-actions">
          <el-button type="primary" size="mini" @click="TryCopyCurrentQRCode()">
            复制二维码图片
          </el-button>
          <el-button size="mini" @click="CopyCurrentQRCodeText()">
            复制原始链接
          </el-button>
        </div>
        <div class="share-qr-tip">
          打开弹窗时会自动尝试复制图片；如果当前浏览器不支持，可直接扫码或复制原始链接。
        </div>
        <el-input
          v-model="qrDialog.text"
          type="textarea"
          :rows="4"
          readonly
        ></el-input>
...
export default {
  ...
  methods: {
    ...
    BuildSubscriptionShare(row) {
      return `https://${row.dispatch_domain}/p?k=${row.id}&t=clash#${encodeURIComponent(row.name)}`;
    },
    BuildVmessShare(row) {
      var shareText = `auto:${row.id}@${row.id}.${row.domain_id}:2443`;
      shareText = Base64.encode(shareText);
      return `vmess://${shareText}?remarks=${encodeURIComponent(row.name)}&alterId=0`;
    },
    ...
    async ShowShareQRCode(row, shareType) {
      var text = "";
      var title = "";
      if (shareType === "vmess") {
        text = this.BuildVmessShare(row);
        title = `[${row.name}] - Vmess 二维码`;
      } else {
        text = this.BuildSubscriptionShare(row);
        title = `[${row.name}] - 订阅二维码`;
      }

      try {
        var imageDataUrl = await this.WithTimeout(
    ...
    async TryCopyCurrentQRCode() {
      var clipboard = navigator.clipboard;
      var ClipboardItemConstructor =
        typeof window !== "undefined" ? window.ClipboardItem : null;

      if (
        !this.qrDialog.imageDataUrl ||
        !clipboard ||
      ...
      try {
        var imageBlob = this.DataUrlToBlob(this.qrDialog.imageDataUrl);
        ...
        this.$message({
          showClose: true,
          message:
            "复制二维码图片失败，请直接扫码或复制原始链接",
          type: "warning",
        });
      }
    },
    CopyCurrentQRCodeText() {
      var t = this;
      this.$copyText(this.qrDialog.text).then(
        function () {
          t.$message({
            showClose: true,
            message: "原始链接已复制到剪切板",
    ...
    CopyShare(row) {
      var r = this.BuildSubscriptionShare(row);
      var t = this;
    ...
    CopyVmessShare(row) {
      var r = this.BuildVmessShare(row);
    ...
    AddOrUpdateUserDNSRecord(row) {
      const formData = new URLSearchParams();
      formData.append("key", this.uif.user_manager.apiPwd);
      formData.append("method", "AddOrUpdateUserDNSRecord");
      formData.append(
        "recordInfo",
        JSON.stringify({
          user_id: row.id,
    ...
    confirmDeleteAction(confirmMessage, onConfirm, errorMessage) {
    ...
    DeleteUserDNSRecord(row) {
      this.confirmDeleteAction(
        `确认删除用户 ${row.id} 的 DNS 解析吗？`,
        () => {
          const formData = new URLSearchParams();
          formData.append("key", this.uif.user_manager.apiPwd);
          formData.append("method", "DeleteUserDNSRecord");
          formData.append(
            "recordInfo",
            JSON.stringify({
              user_id: row.id,
    ...
    Delete(row, table) {
      ...
      const targetText = tableTextMap[table] || "记录";
      this.confirmDeleteAction(
        `确认删除${targetText} ${row.id} 吗？`,
        () => {
          const formData = new URLSearchParams();
          formData.append("key", this.uif.user_manager.apiPwd);
