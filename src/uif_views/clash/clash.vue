    ...
    <el-card class="box-card">
      <div slot="header" class="clearfix">
        <span>
          ...
          <!-- 系统内存： {{ uif.connections.system_info.memory }} -->
          <!-- {{ BuildTotal(InitConnection().downloadTotal) }} -->
          <!-- <el-divider direction="vertical"></el-divider> -->
          <!-- CPU：{{ BuildTotal(InitConnection().memory) }} -->
        </span>
      </div>
...
export default {
  ...
  watch: {
    "uif.connection.system_info": {
      handler(newValue) {
        this.UpdateOSInfo(newValue);
      },
    },
  },
  mounted() {
    this.GetUIFConfig();
    this.UpdateOSInfo(this.uif.connections.system_info);
  },
  created() {},
  methods: {
    ...mapActions({
    ...
    UpdateOSInfo(info) {
      this.UpdateChart("Memory", info.memory.used, info.available);

      if ("cpu_usage" in info) {
        var cpu_usage = info.cpu_usage[0];
    ...
    SortByInbound(a, b) {
      return this.BuildInbound(a).localeCompare(this.BuildInbound(b));
    },
    SortByInfo(a, b) {
      return this.BuildInfo(a).localeCompare(this.BuildInfo(b));
    },
    SortByRoute(a, b) {
      return this.BuildOutboud(a).localeCompare(this.BuildOutboud(b));
    },
    SortByTraffic(a, b) {
