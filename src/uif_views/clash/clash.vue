...
export default {
  ...
  methods: {
    ...
    OpenLocalExternalURL() {
      ...
      var q = `?hostname=${encodeURIComponent(c.apiIP)}&port=${c.apiPort}&secret=${encodeURIComponent(c.apiKey)}`;
      if (c.external_ui_download_url.includes("metacubexd")) {
        u = u + "#/" + q;
      } else {
        u = u + q;
      }
