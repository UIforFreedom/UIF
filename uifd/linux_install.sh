...
download_and_install_system_service(){
  ...
  if [[ $system_service == "systemd" ]]; then
    service_download_path="$service_download_url/uiforfreedom.service"
    service_save_path="/etc/systemd/system/uiforfreedom.service"
  else
    service_download_path="$service_download_url/uiforfreedom.sh"
    service_save_path="/etc/init.d/uiforfreedom.sh"
  fi
...
run_uif(){
  ...
  if [[ $system_service == "systemd" ]]; then
    ...
    /etc/init.d/uiforfreedom.sh restart
  fi
}

stop_uif(){
  if [[ $system_service == "systemd" ]]; then
    systemctl stop uiforfreedom || true
  else
    /etc/init.d/uiforfreedom.sh stop || true
  fi
}
...
check_system_service(){
  if command -v systemctl &> /dev/null; then
    ...
    quit_with_error "Need 'systemd' or 'procd'!"
  fi
