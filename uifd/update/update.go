...
func UpdateSingBoxCore() {
	...
	for o := range osType {
		for a := range arche {
			...
			decompressPath := uif.GetWorkSpace() + "/cores/" + saveOS + "/" + saveArch + "/"
			// 仅当新版本目录存在时才清理旧版，避免误删
			if _, statErr := os.Stat(decompressPath + version); statErr == nil {
				if entries, readErr := os.ReadDir(decompressPath); readErr == nil {
					for _, entry := range entries {
						if entry.IsDir() && entry.Name() != version {
							_ = os.RemoveAll(filepath.Join(decompressPath, entry.Name()))
						}
					}
				}
			}
