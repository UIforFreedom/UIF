...
func Update() string {
	...
	if isNeedUpdate {
		...
		res.Status = 1
		res.Res = "Has No New Version To Update."
		CleanOldData()
	}
...
func CleanOldData() {
	DeleteVersionDir(GetNewestUIFVersion(), GetWorkSpace()+"/uifd/")
	DeleteVersionDir(GetNewestUIFVersion(), GetWorkSpace()+"/webs/")

	DeleteVersionDir(GetNewestCoreVersion(), GetWorkSpace()+"/cores/")
}

func DeleteVersionDir(usingVersion string, dirPath string) {
	err := filepath.WalkDir(dirPath, func(path string, d os.DirEntry, err error) error {
		if err != nil {
			return err
		}
		if d.IsDir() && d.Name() != usingVersion {
			WriteLog(path)
			// os.RemoveAll(path)
		}
		return nil
