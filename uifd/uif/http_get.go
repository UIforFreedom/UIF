...
func HTTPWithPort(dst string, proxyPort string,
	...
	req, err := http.NewRequest(method, dst, b)
	if err != nil {
		return "", "", err
	}
...
func HTTPGetProxy(dst string) (string, string, error) {
	return HTTPWithPort(dst, GetHttpApiPort(), "", "", "")
}

func HTTPGetDirect(dst string) (string, string, error) {
	return HTTPWithPort(dst, GetHttpApiPortDirect(), "", "", "")
}
...
func HTTPMutiple(dst string) *HTTPRes {
	...
	go func() {
		...
		ch <- &HTTPRes{Error: errors.New("Timeout! Failed to get data"), IsTimeout: true}
	...
	go func() {
		r := &HTTPRes{}
		r.Res, r.ExtraMsg, r.Error = HTTPGetDirect(dst)
		ch <- r
	}()

	go func() {
		r := &HTTPRes{}
		r.Res, r.ExtraMsg, r.Error = HTTPGetProxy(dst)
		ch <- r
	}()
